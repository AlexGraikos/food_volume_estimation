...
intrinsics_mat_inv = np.linalg.inv(intrinsics_mat)
intrinsics = K.variable(intrinsics_mat)
intrinsics_inverse = K.variable(intrinsics_mat_inv)
pose_vec = np.array([-doffs*scale, 0, 0, 0, 0 ,0])
pose = K.expand_dims(K.variable(pose_vec), axis=0)

# Reconstruct target view

result, _ = inverse_warp(img, depth, pose, intrinsics, intrinsics_inverse)
