...
from keras.layers import Conv2D, UpSampling2D, GlobalAveragePooling2D, LeakyReLU, Dense, Flatten
...
class MonocularAdversarialModel:
    ...
    def initialize_training(self):
        ...
        loss_list = (['mae' for i in range(2)])

        adam_opt = Adam(lr=10e-4)
        self.generator.compile(adam_opt, loss=loss_list)
    ...
    def create_generator(self):
        ...
        def dec_layer(prev_layer, skip_layer, filters, upsample):
            ...
            return dec
        # Input
        depth_features = Input(shape=depth_encoder_skip.output_shape[0][1:])
        skip6 = Input(shape=depth_encoder_skip.output_shape[1][1:])
        skip5 = Input(shape=depth_encoder_skip.output_shape[2][1:])
        skip4 = Input(shape=depth_encoder_skip.output_shape[3][1:])
        skip3 = Input(shape=depth_encoder_skip.output_shape[4][1:])
        skip2 = Input(shape=depth_encoder_skip.output_shape[5][1:])
        skip1 = Input(shape=depth_encoder_skip.output_shape[6][1:])
        # Layers
        ...
        # Model
        inverseDepth = Conv2D(filters=1, kernel_size=3, strides=1, padding='same', activation='sigmoid')(dec1b)
        ...
        depth_features_curr = Input(shape=depth_encoder.output_shape[1:])
        depth_features_prev = Input(shape=depth_encoder.output_shape[1:])
        depth_features_next = Input(shape=depth_encoder.output_shape[1:])
