...
class MonocularAdversarialModel:
    ...
    def parse_args(self):
        ...
        parser.add_argument('--save_per', type=int, help='Epochs between model saving during training.', default=5)
    ...
    def initialize_training(self):
        ...
        inverseDepths = self.generator.outputs[8:12]
        perScaleReprojections = self.generator.outputs[12:]
        #validationPrev = self.discriminator([self.generator.input[0]] + reprojections[0::2])
        #validationNext = self.discriminator([self.generator.input[0]] + reprojections[1::2])
        self.adversarial_model = Model(inputs=self.generator.input,
            outputs=(perScaleReprojections + perScaleReprojections),
        ...
        loss_list = ([custom_modules.perScaleMinMAE for _ in range(4)] +
                     [custom_modules.perScaleMinSSIM for _ in range(4)])
    ...
    def train(self, train_df_file, batch_size, training_epochs):
        ...
        learning_rate_callback = self.__learning_rate_halving(start_epoch=10, period=5)
        ...
        for e in range(1, training_epochs+1):
            ...
            self.adversarial_model.fit_generator(adversarialDatagen, steps_per_epoch=steps_per_epoch,
                epochs=e, initial_epoch=e-1, verbose=1, use_multiprocessing=True)

            # Save model weights
            if (e % self.args.save_per == 0):
                postfix = '_epoch_' + str(e)
                self.save_model(self.generator, self.model_name, 'weights', postfix)
    ...
    def __set_weights_trainable(self, model, trainable):
        """
        Set model weights to trainable/non-trainable
            Inputs:
    ...
    def save_model(self, model, name, mode, postfix=''):
        ...
        if model is not None:
            ...
            ## Keras bug - cannot save if weights are trainable
            self.__set_weights_trainable(model, False)
            ###################################################

            # Save model weights/architecture
            if mode == 'weights':
                filename = 'trained_models/' + name + '_weights' + postfix + '.h5'
    ...
    def adversarial_datagen(self, training_data_df, height, width, batch_size):
        ...
        while True:
            curr_frame = (curr_generator.__next__() / 127.5) - 1
            prev_frame = (prev_generator.__next__() / 127.5) - 1
            next_frame = (next_generator.__next__() / 127.5) - 1
